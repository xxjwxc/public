...
const (
	...
	_ErrCode_name_4 = "参数无效appid参数无效密文校验失败用户名不存在或密码错误键值对重复未找到无效操作授权码错误已被使用已被激活激活码被禁止使用用户已存在验证码请求过于频繁邮箱发送失败手机发送失败手机号格式有问题验证码错误用户不存在topic已经存在topic不存在bundle_id不存在topic开启处理失败topic处理类型不存在topic不能为空设备不存在"
	_ErrCode_name_5 = "状态已存在上级菜单不存在菜单不存在用户权限不存在设备ID不存在商品处理类型不存在商品不存在商品正在打折商品可支付类型不存在商品已存在"
	_ErrCode_name_6 = "订单不存在商品未打折会话不匹配商品已恢复原价邀请用户不存在邀请用户级数已满用户未授权申请人已存在申请人不存在订单无效微信零钱重复提现提现金额错误微信提现失败重复提交bundle已存在权限已存在权限不存在房型不存在房型已存在房间不存在房间已存在房价代码或房价名称已存在房价代码不存在文件不存在房间未启用班次已存在班次不存在系统时间与营业时间不匹配当前班次已交班支付金额错误存量不足数据存储错误数据添加错误数据更新错误数据删除错误时间错误预定信息错误不允许已过期超过最大值低于最小值已存在未绑定绑定失败计算错误token校验失败应用id未找到已经处理解析文件错误缓存异常模板执行错误请不要平凡操作数据为空用户已取消关注存在关联状态错误超限拒绝"
	_ErrCode_name_7 = "用户名格式错误无效操作,存在使用中的虚拟人被另外一个浏览器打开，请稍后再试资源未找到产品代码重复Excel导出失败未找到发布版本文件保存失败已存在未发布的版本校验资源失败本次发布无效本次发布失败请至少完成一个语种的配置训练失败无效操作,存在使用中的知识库无效操作,存在使用中的指令训练数据已过期，请重新训练知识库训练错误，请稍后再试问答服务错误，请稍后再试知识库聚类错误，请稍后再试获取索引异常索引删除异常NLU测试异常, 请稍后再试NLU置信度获取异常, 请稍后再试NLU训练数据生成错误，请稍后再试NLU训练失败，请稍后再试NLU发布失败，请稍后再试无效操作,存在使用中的模型未找到可用知识库未找到应用信息存在正在使用的应用信息未找到可用的虚拟人模板未找到可用的虚拟人无效操作,空间存在正在使用的数据未找到可用的租户空间请上传完整的角色模型请上传角色骨骼请上传角色动画未匹配到设备未授权无AI扩写数据请先录入FAQ字数超限该母版正在被使用，无法删除。该版本正在被使用，无法删除。该场景正在被使用，无法删除。一次最多导出10万条交互记录，请缩小筛选范围Excel导入失败正在生成辅助问答对，请等待完成后再进行训练请先生成辅助问答对已清理"
)
...
var (
	...
	_ErrCode_index_4 = [...]uint16{0, 12, 29, 47, 80, 95, 104, 116, 131, 143, 155, 179, 194, 221, 239, 257, 281, 296, 311, 328, 342, 360, 383, 409, 426, 441}
	_ErrCode_index_5 = [...]uint8{0, 15, 36, 51, 72, 89, 116, 131, 149, 179, 194}
	_ErrCode_index_6 = [...]uint16{0, 15, 30, 45, 66, 87, 111, 126, 144, 162, 174, 198, 216, 234, 246, 261, 276, 291, 306, 321, 336, 351, 387, 408, 423, 438, 453, 468, 504, 525, 543, 555, 573, 591, 609, 627, 639, 657, 666, 675, 690, 705, 714, 723, 735, 747, 764, 781, 793, 811, 823, 841, 862, 874, 895, 907, 919, 925, 931}
	_ErrCode_index_7 = [...]uint16{0, 21, 61, 109, 124, 142, 159, 180, 198, 225, 243, 261, 279, 315, 327, 367, 404, 443, 482, 518, 557, 575, 593, 625, 666, 711, 744, 777, 814, 838, 859, 892, 925, 952, 998, 1028, 1058, 1079, 1100, 1112, 1127, 1144, 1159, 1171, 1213, 1255, 1297, 1359, 1376, 1439, 1466, 1475}
)
